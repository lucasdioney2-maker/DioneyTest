# Studio 085 — Proposta de Serviços
## Para enviar a empresas interessadas após o primeiro contacto

---

## O QUE OFERECEMOS

### 🌐 FASE 1 — Site Profissional (quase de graça)

Criamos o vosso site do zero, com design profissional adaptado ao vosso negócio.

**O que pagam:**
| Item | Valor |
|------|-------|
| **Criação do site** (design + desenvolvimento) | **150€** (pagamento único) |
| Hospedagem + domínio + manutenção mensal | **50€/mês** |
| Plataforma (Framer) | incluído |

**O que está incluído no site:**
- ✅ Design personalizado ao vosso negócio
- ✅ Página principal, sobre nós, serviços/menu, contactos
- ✅ Optimizado para Google (SEO básico)
- ✅ Adaptado a telemóvel e computador
- ✅ Formulário de contacto
- ✅ Ligação ao vosso Google Maps e Instagram

> **Nota:** O site é visual, limpo e profissional. No futuro, se precisarem de funcionalidades extra (reservas online, área de cliente, loja), podemos adicionar — discutimos os custos nessa altura.

---

### 📸 FASE 2 — Produção de Conteúdo (serviço mensal)

Depois do site no ar, o próximo passo é alimentá-lo com conteúdo de qualidade.

**Pacotes disponíveis:**

#### Pack Básico — 500€/mês
- Sessão fotográfica mensal
- 4 vídeos curtos (reels/stories)
- 8 publicações para Instagram (foto + legenda)
- Atualização de conteúdo no site (1x/mês)

#### Pack Intermédio — 700€/mês
- Sessão fotográfica mensal
- 6 vídeos (reels, stories, apresentação)
- 12 publicações para Instagram
- Atualização de conteúdo no site (2x/mês)
- Relatório mensal de visitas e engagement

#### Pack Completo — 950€/mês
- Sessões de produção ilimitadas no mês
- 8 vídeos (reels, stories, institucionais)
- 30 publicações para Instagram
- Gestão completa do Instagram (respostas, stories, estratégia)
- Atualização de conteúdo no site (semanal)
- Relatório mensal + estratégia de conteúdo

---

### 🤖 FASE 3 — Conteúdo com IA (alternativa aos packs)

Para quem não queira ou não possa investir em produção audiovisual física, oferecemos uma alternativa com Inteligência Artificial:

- Imagens e vídeos gerados por IA (estilo do negócio)
- Textos para Instagram e site gerados automaticamente
- Automação de publicações e respostas

> Esta opção tem custo mais reduzido e pode ser combinada com produção real em datas especiais (lançamentos, promoções).

**Custo:** orçamento personalizado conforme o volume

---

### 🏪 FASE 4 — Expansão do Site (futuro)

Quando o negócio crescer online, podemos adicionar:
- Loja online / e-commerce
- Sistema de reservas e marcações
- Área de cliente com login
- Newsletter automática

**Custo:** orçamento personalizado conforme a funcionalidade

---

## COMO FUNCIONA O PROCESSO

```
1. REUNIÃO INICIAL (gratuita, 30 min)
   ↓ Percebemos o vosso negócio e o que querem comunicar

2. DEMONSTRAÇÃO (gratuita)
   ↓ Mostramos um protótipo do site no Framer — vêem o resultado antes de decidir

3. APROVAÇÃO & LANÇAMENTO (1-2 semanas)
   ↓ Ajustes finais → site no ar com o vosso domínio

4. PARCERIA DE CONTEÚDO (mensal, opcional)
   ↓ Foto, vídeo, Instagram, site actualizado
```

---

## GARANTIAS

- ✅ Sem contracto longo — podem sair quando quiserem
- ✅ O site fica vosso — domínio e conteúdo pertencem ao negócio
- ✅ Suporte técnico incluído nos primeiros 3 meses
- ✅ Revisões ilimitadas durante a criação

---

## PORQUÊ O STUDIO 085?

Somos um estúdio pequeno, mas comprometido. Não temos dezenas de clientes simultâneos — cada parceria é tratada com atenção personalizada. O nosso objectivo é crescer junto com os negócios que escolhemos apoiar.

---

**Studio 085 | Porto**
📞 +351 913 137 568
📷 @studio085pt | @dioneyfilmes
✉️ studio085pt@gmail.com
🌐 https://www.canva.com/design/DAGoqWC6gAo/cuPjAi8B7VeOpF8G0GiIhw/view?utm_content=DAGoqWC6gAo&utm_campaign=designshare&utm_medium=link2&utm_source=uniquelinks&utlId=h35fe059f43#2
